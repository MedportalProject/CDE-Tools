...
namespace CDETools.Entity.Bioportal
{
    public static class BioportalNetwork
    {
        public static string GetJsonResponse(string url)
        {
            HttpClient web = new HttpClient();
            //Bioportal在进行查询时候需要加身份验证
            //please replace your own bioportal APIKEY
            web.DefaultRequestHeaders.Add("Authorization", "apikey token=YOUR OWN BIOPORTAL APIKEY");
        ...
        public static T GetResponseAs<T>(string url)
        {
            ...
            //MessageBox.Show(JsonConvert.DeserializeObject<T>(jsonResponse).ToString());
